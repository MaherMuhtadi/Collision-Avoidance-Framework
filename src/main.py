...
def kill_zombie_carla_processes():
    """Terminate any existing CARLA processes."""
...
def next_save_file(base_dir="SensorData", prefix="run_"):
    """Generate next available shelve file path."""
...
def save_lidar_settings(lidar):
    """Save LiDAR sensor configuration to JSON file."""
    try:
...
def main():
    """Run CARLA simulation and collect sensor data."""
    global running
    ...
    def log_collision(event):
        """Log collision event and stop simulation."""
        global running
